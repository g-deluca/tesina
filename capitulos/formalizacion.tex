...
archivo \texttt{ValidityInvariance.v}. Consiste, fundamentalmente, en un análisis por cosas en la
...
\begin{theorem} [Las ejecuciones preservan la validez del estado]
    ...
    \mbox{}\\
    $\begin{array}{l}
            \forall\ (s\ s':\AndroidState)(a:\Action) (r:Response),
            s\step{a/r}s' \rightarrow valid\_state(s')
        \end{array}$
\end{theorem}

Demostrar este tipo de invariantes del sistema resulta muy útil para luego estudiar otro tipo de
propiedades del mismo. En particular, todas las propiedades que presentaremos a continuación fueron
analizadas solo en estados válidos del modelo.
